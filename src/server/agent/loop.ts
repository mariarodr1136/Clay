...
- kpi: a single aggregated figure (aggregate "count" | "sum" | "avg" over the rows, optional field). Optional note (short caption under the value) and intent "danger" for figures like overdue counts where non-zero is bad. Optional trend { field, goodDirection } draws a sparkline and a period-over-period delta from the tile's OWN bound rows — so it only works on a query that returns an ordered series over time (velocityByWeek, cycleTimeByWeek, completionsOverTime, createdVsCompleted), and field must be a numeric column on those rows. goodDirection says which way is good: "up" for velocity, "down" for cycle time. Don't add a trend to a tile bound to anything else — it will be rejected, because a trend arrow over unordered rows is a claim the data can't support.
